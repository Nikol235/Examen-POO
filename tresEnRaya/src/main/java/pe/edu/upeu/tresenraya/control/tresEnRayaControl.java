...
import pe.edu.upeu.tresenraya.servicios.tresEnRayaServiciosIn;
...
public class    tresEnRayaControl {
    ...
    @Autowired
    tresEnRayaServiciosIn serviciosIn;

    @FXML
    Label label_turno;

    @FXML
    TableView<tresEnRayaModelo> tableView;
    private ObservableList<tresEnRayaModelo> datos;
    List<tresEnRayaModelo> lista;

    @FXML
    TableColumn<tresEnRayaModelo, String> partidox;

    @FXML
    TableColumn<tresEnRayaModelo, String> jugador1x;

    @FXML
    TableColumn<tresEnRayaModelo, String> jugador2x;

    @FXML
    TableColumn<tresEnRayaModelo, String> ganadorx;

    @FXML
    TableColumn<tresEnRayaModelo, Integer> puntuacionx;

    @FXML
    TableColumn<tresEnRayaModelo, String> estadox;
    ...
    private void ganar(){
        ...
        tresEnRayaModelo to = new tresEnRayaModelo();
        to.setPartido(String.valueOf("1"));
        to.setJugador1(jugador1);
        to.setJugador2(jugador2);
        to.setGanador(jugador1);
        to.setEstado("Terminado");
        if(indexID!=-1){
            serviciosIn.update(to,indexID);
        }else{
            serviciosIn .save(to);
        }
        cuadro();
    ...
    public void  cuadro() {
        ...
        lista = serviciosIn.findAll();

        tableView.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);

        partidox.setCellValueFactory(new PropertyValueFactory<tresEnRayaModelo, String>("partido"));
        partidox.setCellFactory(TextFieldTableCell.<tresEnRayaModelo>forTableColumn());

        jugador1x.setCellValueFactory(new PropertyValueFactory<tresEnRayaModelo, String>("jugador1"));
        jugador1x.setCellFactory(TextFieldTableCell.<tresEnRayaModelo>forTableColumn());

        jugador2x.setCellValueFactory(new PropertyValueFactory<>("jugador2"));
        jugador2x.setCellFactory(TextFieldTableCell.<tresEnRayaModelo>forTableColumn());

        ganadorx.setCellValueFactory(new PropertyValueFactory<>("jugador2"));
        ganadorx.setCellFactory(TextFieldTableCell.<tresEnRayaModelo>forTableColumn());

        estadox.setCellValueFactory(new PropertyValueFactory<>("estado"));
        estadox.setCellFactory(TextFieldTableCell.<tresEnRayaModelo>forTableColumn());
